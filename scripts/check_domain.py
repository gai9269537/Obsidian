#!/usr/bin/env python3
"""
Query DataHub GraphQL API to fetch a dataset's domain association and print it.

Usage:
  python3 scripts/check_domain.py <dataset_urn> [<gms_url>]

If no GMS url is provided, defaults to http://localhost:8080
"""
import sys
import json
...
def graphql_query(gms_url: str, query: str, variables: dict):
    ...
    try:
        ...
        try:
            ...
            body = ''
        print(f'HTTPError {e.code} when querying {url}:', body, file=sys.stderr)
        raise
    except URLError as e:
        print(f'URLError when querying {url}: {e}', file=sys.stderr)
        raise


def print_domain_for_dataset(urn: str, gms_url: str, json_only: bool = False):
    ...
    variables = {"urn": urn}
    resp = graphql_query(gms_url, query, variables)
    if resp is None:
        print('No response from GraphQL', file=sys.stderr)
        return 1
    ...
    if not dataset:
        print('Dataset not found (GraphQL returned no dataset).')
        return 3
